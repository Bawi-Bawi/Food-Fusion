...
<div class="container">
  <h1 class="display-5 fw-bold text-center">Educational Resources</h1>
  <p class="lead text-center">Enhance your cooking skills with our comprehensive collection of downloadable guides, video tutorials, recipe cards, and educational materials.</p>
    ...
    <div class=" d-flex justify-content-center gap-3 mb-4">
        <a href="{{ route('resources', ['name'=>'Educational Resources','filter' => 'all']) }}" class=" btn border rounded-4 {{ request('filter','all') === 'all' ? 'btn-warning' : '' }}">All</a>
        <a href="{{ route('resources', ['name'=>'Educational Resources','filter' => 'video']) }}" class=" btn border rounded-4 {{ request('filter') === 'video' ? 'btn-warning' : '' }}">Videos</a>
        <a href="{{ route('resources', ['name'=>'Educational Resources','filter' => 'pdf']) }}" class=" btn border rounded-4 {{ request('filter') === 'pdf' ? 'btn-warning' : '' }}">PDF Files</a>
         <a href="{{ route('resources', ['name'=>'Educational Resources','filter' => 'infographic']) }}" class=" btn border rounded-4 {{ request('filter') === 'infographic' ? 'btn-warning' : '' }}">Infographic</a>
    </div>
            ...
            <div class="recipe-card mt-4">
              <div class="recipe-image">
                ...
                <img src="{{ asset('storage/image/'.$resource->image) }}"
                     class="img-fluid w-100"
                     style="height: 200px">
                 <span class="badge-difficulty">Infographic</span>
                @endif
              </div>
              <div class="recipe-body">
                ...
                @else
                <a class="view-btn btn" href="{{ asset('storage/image/'.$resource->image)}}" target="_blank"><i class="fa-solid fa-eye"></i> View </a>
                @endif
              </div>
            </div>
        @endforeach
